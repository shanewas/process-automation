...
async function run_bot(e, BROWSER, mainWindow, PARAMS) {
  for (i = 0; i < PARAMS.BOT_PROCESS_GROUPS.length; i++) {
    ...
    PARAMS.GROUP_PROCESS_INDEX =
      PARAMS.BOTS.groups[PARAMS.BOT_PROCESS_GROUPS[i]]["processesIdx"];
    for (j = 0; j < PARAMS.GROUP_ITERATION; j++) {
      let isLoading = false;
      let autoLoad = false;
      let page = await PIE.getPage(BROWSER, loadingWindow, false).catch(
        (err) => {
          isLoading = true;
        }
      );
      if (PARAMS.RUNINGSTATUS && !isLoading) {
        ...
        if (PARAMS.IDX < PARAMS.ITERATION) {
          ...
          // element = PARAMS.BOTPROCESS.processSequence[
          //   PARAMS.BOTS.groups[PARAMS.BOT_PROCESS_GROUPS[i]]["processesIdx"]
          // ];
